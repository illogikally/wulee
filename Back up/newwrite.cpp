...
int main(int argc, char** argv) {
    ...
    vector<vector<uint8_t>> imgData = readImage("image/lena.bmp");
    vector<vector<uint8_t>> key{{0, 1, 0},
                                {1, 1, 1},
                                {0, 1, 0}};
    string binaryMessage = readMessage(string("There would be so much awesome!"));
...
void stego(vector<vector<uint8_t>> stegoBlock, vector<vector<uint8_t>> key, vector<vector<uint8_t>>& imgData, int currentBlock, int directiveNum) {
